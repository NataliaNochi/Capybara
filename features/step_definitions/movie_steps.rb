...
#remoção de filmes_____________________________________

Dado("que {string} faz parte do catálogo") do |movie_name|
  steps %{
    Dado que "#{movie_name}" é um novo filme
...
Quando("confirmo a solicitação") do
  @movie_page.sweet_alert.confirm_removal
end
...
Quando("cancelo a solicitação") do
  @movie_page.sweet_alert.cancel_removal
end
